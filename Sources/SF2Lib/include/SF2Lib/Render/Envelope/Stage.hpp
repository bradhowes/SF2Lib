...
public:
  ...
  void setDecay(int durationInSamples, Float sustainLevel) noexcept {
    ...
    if (durationInSamples_ == 1) {
      ...
      /*
       ...
       decay phase." This implies that if the sustainLevel is higher than -100 dB, then the slope is the same, but the
       duration is shortened to the time required to reach the sustain level.
       ...
       */
      increment_ = -1.0 / durationInSamples_;
      durationInSamples_ = std::max(static_cast<int>(floor((1.0 - sustainLevel) / -increment_)), 0);
      increment_ = (1.0 - sustainLevel) / -durationInSamples_;
    }
  }
  ...
  void setRelease(int durationInSamples, Float sustainLevel) noexcept {
    durationInSamples_ = std::max(durationInSamples, 1);
    increment_ = -1.0 / durationInSamples_;
    durationInSamples_ = std::max(static_cast<int>(ceil(sustainLevel / -increment_)), 0);
  }
  ...
  /**
   ...
   @returns new value
   */
  constexpr Float next(Float last) const noexcept { return DSP::clamp(last + increment_, 0.0f, 1.0f); }
