...
Stage::setAttack(int durationInSamples) noexcept
{
  ...
  increment_ = 1.0_F / durationInSamples_;
}
...
Stage::setDecay(int durationInSamples, Float sustainLevel) noexcept
{
   ...
   */
  auto span = (1.0_F - sustainLevel);
  if (span == 0.0_F) {
    increment_ = 0.0_F;
    return;
  }

  auto increment = -1.0_F / durationInSamples_;
  durationInSamples_ = std::max(static_cast<int>(floor(span / -increment)), 1);
  increment_ = (1.0_F - sustainLevel) / -durationInSamples_;
}
