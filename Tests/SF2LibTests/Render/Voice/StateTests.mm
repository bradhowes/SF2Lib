...
- (void)testInit {
  auto state{self.contexts->context2.makeState(0, 69 + 24, 64)};
...
- (void)testKey {
  ...
  self.sst.setValue(state, Index::forcedMIDIKey, 1);
...
- (void)testVelocity {
  ...
  self.sst.setValue(state, Index::forcedMIDIVelocity, 0);
  XCTAssertEqual(0, state.velocity());
}

- (void)testModulatedValue {
  State::State state{self.contexts->context2.makeState(0, 60, 32)};
  self.sst.setValue(state, Index::holdVolumeEnvelope, 100);
  self.sst.setAdjustment(state, Index::holdVolumeEnvelope, 0);
  XCTAssertEqualWithAccuracy(100.0, state.modulated(Index::holdVolumeEnvelope), 0.000001);
  self.sst.setAdjustment(state, Index::holdVolumeEnvelope, 50);
...
- (void)testInvalidModulatorIsIgnored {
  ...
  self.sst.addModulator(state, badMod1);
  XCTAssertEqual(size_t(10), state.modulatorCount());
}
