...
- (void)setUp {
  ...
  self.state = new State::State(44100.0);
}
...
- (void)testKeyVelocityToInitialAttenuation {
  ...
  XCTAssertEqualWithAccuracy(modulator.value(*self.state, *self.channelState), 0.0, self.epsilon);

  self.sst.setValue(*self.state, Index::forcedMIDIVelocity, 64);
  XCTAssertEqualWithAccuracy(modulator.value(*self.state, *self.channelState), 119.049498788827904, self.epsilon);

  self.sst.setValue(*self.state, Index::forcedMIDIVelocity, 1);
  XCTAssertEqualWithAccuracy(modulator.value(*self.state, *self.channelState), 841.521488382382699, self.epsilon);
...
- (void)testKeyVelocityToFilterCutoff {
  ...
  State::Modulator modulator{config};
  self.sst.setValue(*self.state, Index::forcedMIDIVelocity, 127);
  XCTAssertEqualWithAccuracy(modulator.value(*self.state, *self.channelState), -18.75, self.epsilon);

  self.sst.setValue(*self.state, Index::forcedMIDIVelocity, 64);
  XCTAssertEqualWithAccuracy(modulator.value(*self.state, *self.channelState), -1200.0, self.epsilon);

  self.sst.setValue(*self.state, Index::forcedMIDIVelocity, 1);
  XCTAssertEqualWithAccuracy(modulator.value(*self.state, *self.channelState), config.amount() * 127.0 / 128.0, self.epsilon);
}
...
- (void)testChannelPressureToVibratoLFOPitchDepth {
  ...
  XCTAssertEqualWithAccuracy(modulator.value(*self.state, *self.channelState), 25.0, self.epsilon);
...
- (void)testCC7ToInitialAttenuation {
  ...
  XCTAssertEqualWithAccuracy(modulator.value(*self.state, *self.channelState), 960.0, self.epsilon);
...
- (void)testCC10ToPanPosition {
  ...
  XCTAssertEqualWithAccuracy(modulator.value(*self.state, *self.channelState), -500, self.epsilon);

  self.channelState->setContinuousControllerValue(MIDI::ControlChange::panMSB, 64);
  XCTAssertEqualWithAccuracy(modulator.value(*self.state, *self.channelState), 0, self.epsilon);

  self.channelState->setContinuousControllerValue(MIDI::ControlChange::panMSB, 127);
  XCTAssertEqualWithAccuracy(modulator.value(*self.state, *self.channelState),
...
- (void)testCC91ToReverbSend {
  ...
  XCTAssertEqualWithAccuracy(modulator.value(*self.state, *self.channelState), 100.0, self.epsilon);
...
- (void)testPitchWheelToInitialPitch {
  ...
  self.channelState->setPitchWheelValue(0);
  XCTAssertEqualWithAccuracy( modulator.value(*self.state, *self.channelState), -12600.78125, self.epsilon);
  self.channelState->setPitchWheelValue(4096);
  XCTAssertEqualWithAccuracy( modulator.value(*self.state, *self.channelState), 0.0, self.epsilon);
  self.channelState->setPitchWheelValue(SF2::MIDI::ChannelState::maxPitchWheelValue);
  XCTAssertEqualWithAccuracy( modulator.value(*self.state, *self.channelState), 12597.7048873901367, self.epsilon);
}
...
- (void)testKeyValueProvider {
  ...
  XCTAssertEqualWithAccuracy(0.0, mod.value(*self.state, *self.channelState), self.epsilon);
  self.sst.setValue(*self.state, Index::forcedMIDIKey, 64);
  XCTAssertEqualWithAccuracy(1.5, mod.value(*self.state, *self.channelState), self.epsilon);
  self.sst.setValue(*self.state, Index::forcedMIDIKey, 127);
  XCTAssertEqualWithAccuracy(2.9765625, mod.value(*self.state, *self.channelState), self.epsilon);
}
